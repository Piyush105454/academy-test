    ...
    async function loadBatchData() {
      ...
      try {
        ...
        if (studentsList.length > 0) {
          ...
          // Helper for current month completion check
          const today = new Date();
          const currentMonthIdx = today.getMonth();
          const lastDayOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
          const isCurrentMonthEnded = today.getDate() === lastDayOfMonth;

          // Apply earnings filter (exclude ongoing month premature attendance bonuses)
                        ...
                        {[...classStudentsList]
                          ...
                          .map((student, i) => (
                            <div key={`earn-${student.id}`} className="flex items-center justify-between group">
                              ...
                              <div className="flex items-center gap-1 text-emerald-600 font-bold text-xs bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100 shrink-0">
